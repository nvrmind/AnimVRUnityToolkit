...
{
    const string currentVersion = "9.0.5";
    ...
    static void Update()
    {
        ...
        if (!gotVersion)
        {
            ...
            if (ShouldDisplay())
            {
                ...
                window = GetWindow<AnimVRUpdate>(true);
                window.minSize = new Vector2(320, 440);
                window.titleContent = new GUIContent("AnimVR Unity Toolkit");
            }
        }
    ...
    {
        var ms = MonoScript.FromScriptableObject(window);
        var path = AssetDatabase.GetAssetPath(ms);
        path = Path.GetDirectoryName(path);
        return File.ReadAllText(path.Substring(0, path.Length - "Editor".Length) + "version.txt");
    }
    ...
    public void OnGUI()
    {
        EditorGUILayout.HelpBox("A new version of the AnimVR Unity Toolkit is available!", MessageType.Warning);
